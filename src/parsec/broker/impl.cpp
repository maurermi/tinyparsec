...
namespace cbdc::parsec::broker {
    impl::impl(runtime_locking_shard::broker_id_type broker_id,
               // std::vector<std::shared_ptr<runtime_locking_shard::interface>>
               // shards, std::shared_ptr<ticket_machine::interface> ticketer,
    ...
    auto impl::begin(begin_callback_type result_callback) -> bool {
        // don't need any of this, just increment the ticket number
        // return 1000 tickets (default)
        // in a pair
        // m_ticket_mut.lock();
        // // this might be unnecessary
        // m_tickets.emplace(
        //     m_ticket_machine,
        //     std::make_shared<state>(state{ticket_state::begun, {}}));
        // result_callback(m_ticket_machine);
        // m_ticket_machine += 1000;
        // if(m_highest_ticket < m_ticket_machine) {
        //     m_highest_ticket = m_ticket_machine;
        // }
        // // call agent->do_begin
        // m_ticket_mut.unlock();
    ...
    //     const
    //     parsec::runtime_locking_shard::interface::try_lock_return_type&
    //         res) {
    ...
    //                 m_log->trace(this, "Broker locked key for",
    //                 ticket_number);
    ...
    auto impl::try_lock(key_type key,
                        value_type value,
                        lock_type locktype,
        ...
        auto maybe_error = [&]() -> std::optional<error_code> {
            ...
            // l.lock();
            if(locktype
               == lock_type::write) { // just write if the lock can be acquired
                m_shard[key] = value;
            } else { // should something be done here? -- probably should do
                     // something with this data
                assert(value.size() == 0);
                if(m_shard.count(key) <= 0) { // change to .contains in c++ 20
                    // this might be incorrect ^
                    return error_code::
                        directory_unreachable; // the key isn't in the
                                               // dictionary -- this might be
                                               // wrong
                }
            }
            result_callback(m_shard[key]);
            // l.unlock();
            // auto it = m_tickets.find(ticket_number);
            // if(it == m_tickets.end()) {
            //     return error_code::unknown_ticket;
            ...
            //                            res);
            //        })) {
            //     m_log->error("Failed to make key location directory
            //     request"); return error_code::directory_unreachable;
            // }
    ...
    //         if(!m_shards[sidx]->commit( // need to modify because m_shards
    //         is
    //                                     // no longer valid
    //                ticket_number,
    //                [=](const parsec::runtime_locking_shard::interface::
    //                        commit_return_type& comm_res) {
    //                    handle_commit(commit_cb, ticket_number, sidx,
    //                    comm_res);
    //                })) {
    ...
    //                      " "with success for", ticket_number);
    //         commit_cb(std::nullopt);
    //     }
    // }

    // fix this
    auto impl::commit(state_update_type state_updates,
                      commit_callback_type result_callback) -> bool {
        m_log->trace(this, "Broker got commit request");
        // just update the map don't try to contact the shards
        // state update type = unordered_map<key_type, value_type,
        // hashing::const_sip_hash<key_type>>;

        std::unique_lock l(m_mut);
        // l.lock();
        for(auto i = state_updates.begin(); i != state_updates.end();
            i++) { // new
            m_shard[i->first] = i->second;
        }
        ...
        //                 m_log->error("Cannot commit, still waiting for
        //                 locks"); return error_code::waiting_for_locks;
        //             }
        //         }
        //     }
    ...
    //     m_log->trace(this, "Broker got rollback request for",
    //     ticket_number); auto callback = false; auto maybe_error = [&]() ->
    //     std::optional<error_code> {
    //         std::unique_lock l(m_mut);
    //         auto it = m_tickets.find(ticket_number);
    //         if(it == m_tickets.end()) {
    ...
    //     { auto callback = false; auto maybe_error = [&]() ->
    //     std::optional<error_code> {
    //         std::unique_lock lll(m_mut);
    //         auto ittt = m_tickets.find(ticket_number);
    //         if(ittt == m_tickets.end()) {
    ...
    //         //     case ticket_state::aborted:
    //         //         return error_code::aborted;
    //         //} // remove this ? maybe not ?

    //         // if(!res.has_value()) {
    //         //     return error_code::directory_unreachable;
    //         // }

    //         // ---- might need this block -----
    //         // auto& ss = tss->m_shard_states[shard_idx]; // modify this ?
    ...
    //     m_log->trace(this, "Broker handled get_tickets for shard",
    //     shard_idx);
    // }
    ...
    //                     handle_recovery_commit(result_callback, tn,
    //                     comm_res);
    //                 });
    //             if(!success) {
    //                 return error_code::shard_unreachable;
    ...
    // void
    // impl::handle_recovery_commit(const recover_callback_type&
    // result_callback,
    //                              ticket_number_type ticket_number,
    ...
    // void
    // impl::handle_recovery_finish(const recover_callback_type&
    // result_callback,
    //                              finish_return_type res) {
    //     if(res.has_value()) {
